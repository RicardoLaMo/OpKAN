...
% -----------------------------------------------------------------------
\begin{abstract}
Physics-informed Kolmogorov-Arnold Networks (KANs) introduce a management problem that
gradient descent cannot solve: \emph{when should an edge remain a B-spline, be replaced
by a symbolic function, or be pruned?} We formalize this as the \emph{topology surgery
problem} and show it produces five chronic failure modes in heterogeneous KANs ---
topology stagnation, parameterization lock-in, symbolic-numerical tension, regime
blindness, and $C^2$ instability --- each arising from a distinct gradient-signal gap.
To address this, we propose a \textbf{dual-process topology agent}: a fast reflexive
controller performs high-frequency local maintenance (pruning, learning-rate
stabilization); a slow strategic controller performs low-frequency topology surgery
guided by PDE geometry and market-regime signals, separating the two tasks that
gradient descent conflates. A typed DSL, static AST allowlist, and atomic rollback
guarantee that every mutation preserves $C^2$ continuity and transactional integrity.
Demonstrated on a Heston stochastic volatility PI-KAN, where the cross-derivative
term imposes a hard $C^2$ requirement: ablation shows System~2 is \emph{necessary}
for regime-ordered behavior, System~1 is \emph{necessary} for sustained throughput,
and PDE residual minimization alone fails all five financial monotonicity checks under
distributional shift --- a training-objective insufficiency result that generalizes to
any physics-constrained network under regime change. The system sustains
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{$C^2$ instability.} $C^2$ instability refers specifically to the
...
network trained without explicit regime-ordering terms in the objective. The
...
System~2's strategic reasoning: neither gradient-only training nor System~1 reflexive
pruning produces regime-ordered behavior (0/5 pass rate in both cases). This confirms
that monotonicity is a product of System~2's chain-of-thought reasoning over PDE
...
novelty is that the ``controller'' is an LLM reasoning in natural language about
