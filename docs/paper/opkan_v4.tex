...
% -----------------------------------------------------------------------
\begin{abstract}
Kolmogorov-Arnold Networks (KANs) enable surgical symbolic replacement of individual
edges at runtime, creating a class of management problem gradient descent cannot solve:
\emph{topology parameterization selection}. We identify five gradient-signal gaps in
heterogeneous KAN topology (B-spline $\cup$ symbolic edges) and show they decompose
across two cognitive timescales. A \textbf{dual-process topology agent} addresses
them: System~1 (7B model, $<$50\,ms) handles high-frequency reflexive maintenance via
statistical thresholding; System~2 (32B model, 200--500\,ms) handles low-frequency
strategic surgery via chain-of-thought reasoning over PDE geometry. A typed DSL with
AST allowlist and atomic rollback provides $C^2$-preserving vocabulary restriction and
transactional integrity. Demonstrated on a physics-informed KAN solving the Heston
stochastic volatility PDE: an ablation study shows System~2 is necessary for
regime-ordered behavior, System~1 is necessary for sustained throughput, and PDE
residual minimization alone is insufficient for monotonicity under distributional shift
--- a result that generalizes beyond options pricing to any physics-constrained network
deployed under regime change. Sustained throughput: 26,300\,smp/sec on H200; zero GPU
blocking. The controller is currently prompt-engineered; a trajectory-collection
infrastructure for future policy fine-tuning is in place.
\end{abstract}
