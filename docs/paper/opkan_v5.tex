...
\begin{abstract}
Physics-informed Kolmogorov-Arnold Networks (KANs) introduce a structural decision
problem that gradient descent cannot solve: when should an edge remain a learnable
spline, be replaced by a symbolic function, or be pruned to zero? We formalize this
as the \emph{topology surgery problem} and identify five chronic failure modes in
heterogeneous KANs, each arising from a distinct gradient-signal gap: topology
stagnation, parameterization lock-in, symbolic-numerical tension, regime blindness,
and activation instability under second-order derivative constraints. To address this,
we propose a \textbf{dual-process topology agent}: a fast reflexive controller performs
high-frequency local maintenance (pruning, learning-rate stabilization); a slow
strategic controller performs low-frequency topology surgery guided by PDE geometry
and market-regime signals, separating the two structural decisions that gradient
descent cannot distinguish. A typed domain-specific language, static expression
validator, and atomic rollback guarantee that every mutation preserves activation
smoothness and transactional integrity. Demonstrated on a Heston stochastic volatility
PI-KAN, where exact second derivatives are required by the cross-derivative term: an
ablation study shows System~2 is required for regime-ordered behavior, while System~1
restores full throughput without sacrificing those guarantees. PDE residual
minimization alone fails all five financial monotonicity checks under distributional
shift, revealing a training-objective insufficiency that may extend to other
physics-constrained networks under regime change. The system sustains
$\mathbf{26{,}300}$\,smp/sec on H200 with zero GPU blocking, showing that online
structural governance need not compromise training efficiency.
\end{abstract}
...
topology management. Each failure mode corresponds to a distinct gradient-signal gap,
a regime where the training objective provides no corrective gradient for the
relevant structural decision (Table~\ref{tab:problems}).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{$C^2$ (twice-differentiable) instability.} $C^2$ instability refers specifically to the
  condition where aggressive learning-rate updates drive B-spline coefficient updates
  that increase $\|\partial^2\phi_e/\partial x^2\|_\infty$ beyond the tolerance for
  stable autograd evaluation of the Heston cross-derivative term
  $\rho\sigma vS\,\partial^2 V/\partial S\partial v$. The symptom is NaN propagation
  in the cross-derivative computation, detectable by monitoring the L2 norm of the
...
decision requires strategic structural reasoning over PDE structure.
